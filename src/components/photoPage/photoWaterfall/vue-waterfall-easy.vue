...
<!-- 'padding-top':gap/2+'px', 'padding-left':gap/2+'px', -->
...
<template lang="pug">
.vue-waterfall-easy(
  :style="isMobile? '':{width:colWidth*columnCount+'px',height:'100%',left:'50%',marginLeft: -1*colWidth*columnCount/2 +'px'}"
)
  a.img-box(
...
export default {
  ...
  computed: {
    colWidth() { // 每一列的宽度
      return this.imgWidth + this.gap/2
    },
    imgWidthC() { // 对于移动端重新计算图片宽度
      return this.isMobile ? (window.innerWidth - 24 - 6) / 2 : this.imgWidth
    }
  },
  methods: {
    imgClick(src,i){
